...
//include_once  $directory . '/WIInc/edit/' . $page_id . '.php' ;
      $sql = "SELECT * FROM `wi_page` WHERE `name`=:page_id ";
      $query = $this->WIdb->prepare($sql);
      $query->bindParam(':page_id', $page_id, PDO::PARAM_STR);
      $query->execute();

      $res = $query->fetch(PDO::FETCH_ASSOC);

      $contents = $res['contents'];
      //echo 'WIModule/' .$contents.'/'.$contents.'.php';
      require_once  '../../WIModule/' .$contents.'/'.$contents.'.php';
        
       // echo $mod;
        $contents = new $contents;

       // $mod->mod_name();
      $contents->editPageContent($page_id);
      //$res->closeCursor();
