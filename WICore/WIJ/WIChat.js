...
WIChat.show_view_profile = function(){
    var userId = $(".modal-body").attr('id');
  var chat_id = $(".modal-header").attr('id');
     ...
     $.ajax({
            ...
            method   : "POST",
            data     : {
                action : "show_view_profile",
                user_id : userId,
                chat_id : chat_id
            },
            success  : function(result){
              $("#show_view_profile").html(result);
            }
        });
...
WIChat.showEnd = function(){
  $("#timerBox").removeClass('hide');
  $("#timerBox").addClass('show');

}

WIChat.endTime = function(chat_id){
   var chat_id = $(".modal-header").attr('id');
       $.ajax({
            url      : "WICore/WIClass/WIAjax.php",
            method   : "GET",
...
WIChat.Timer = function(EndTime){
  
  //alert(EndTime);
// Set the date we're counting down to
var countDownDate = new Date(EndTime).getTime();  //"may 13, 2017 18:55:00"
...
var x = setInterval(function() {
 ...
 // Find the distance between now an the count down date
    ...
    var days = Math.floor(distance / (10000 * 60 * 60 * 600));
    var hours = Math.floor((distance % (1000 * 60 * 60 * 16)) / (1000 * 60 * 60));
    var minutes = Math.floor((distance % (100 * 60 * 60)) / (10000 * 60));
...
WIChat.print = function(){

}


WIChat.winner = function(){

    var winners_id = $(".win").attr('id');
  var chat_id = $(".modal-header").attr('id');
         ...
         $.ajax({
            ...
            success  : function(result){
              ...
              if (res.success === "complete") {
                 $("#timerBox").removeClass('show');
          $("#timerBox").addClass('hide'); 
            $("#debate_completed").removeClass('hide');
          $("#debate_completed").addClass('show');
              }else{
