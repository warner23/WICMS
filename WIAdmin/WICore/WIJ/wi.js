...
const wi_pageBuilder = new window.WIPageBuilder(wi_pageBuilderOpts);
console.log(wi_pageBuilder);
let editing = true;
...
toggleEdit.onclick = evt => {
  ...
  if (editing) {
    wi_pageBuilder.render(renderContainer);
...
viewDataBtn.onclick = evt => {
  console.log(wi_pageBuilder.formData);
};


let wi_pageBuilderLocale = window.sessionStorage.getItem('wi_pageBuilder-locale');
if (wi_pageBuilderLocale) {
  localeSelect.value = wi_pageBuilderLocale;
}

localeSelect.addEventListener('change', function() {
  window.sessionStorage.setItem('wi_pageBuilder-locale', localeSelect.value);
  wi_pageBuilder.i18n.setLang(localeSelect.value);
});

document.getElementById('control-filter')
.addEventListener('input', function(e) {
  wi_pageBuilder.controls.actions.filter(e.target.value);
});

if (isSite) {
  ((window.gitter = {}).chat = {}).options = {
    room: 'draggable/wi_pageBuilder'
  };
